...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    line_params[0] *= scale_factor;
    line_params[1] *= scale_factor;
    line_params[2] *= scale_factor;
    ...
    if (outliers_fraction > 0.0 && !use_huber) {
        threshold *= 45.0; // ослабляем порог если есть выбросы и мы к ним не устойчивы (не робастны за счет loss-функции (функции потерь) Huber-а)
    }
    for (int d = 0; d < 3; ++d) {
