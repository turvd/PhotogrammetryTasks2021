                                 ...
                                 unsigned int n_query_desc)
{
    ...
    for (int query_local_i = 0; query_local_i < KEYPOINTS_PER_WG; ++query_local_i) {
        query_local[query_local_i * NDIM + dim_id] = (query_id0 + query_local_i < n_query_desc) ? query[(query_id0 + query_local_i) * NDIM + dim_id] : 0.0f;
    }
    ...
    for (int train_idx = 0; train_idx < n_train_desc; ++train_idx) {
        ...
        for (int query_local_i = 0; query_local_i < KEYPOINTS_PER_WG; ++query_local_i) {
            ...
            float dist_dim = train_value_dim - query_local[query_local_i * NDIM + dim_id];
            float dist2_dim = dist_dim * dist_dim;
            dist2_for_reduction[dim_id] = dist2_dim;

            // суммируем все что есть в dist2_for_reduction
            barrier(CLK_LOCAL_MEM_FENCE);
            int step = NDIM / 2;
            while (step > 0) {
                if (dim_id < step) {
                    dist2_for_reduction[dim_id] = dist2_for_reduction[dim_id] + dist2_for_reduction[dim_id + step];
                }
                barrier(CLK_LOCAL_MEM_FENCE);
                step /= 2;
            }
            ...
            if (dim_id == 0) {
                ...
                if (dist2 <= res_distance2_local[query_local_i * 2 + BEST_INDEX]) {
                    ...
                    // заменяем самое лучшее сопоставление для локального дескриптора
    ...
    if (dim_id < KEYPOINTS_PER_WG * 2) { // полагаемся на то что нам надо прогрузить KEYPOINTS_PER_WG*2==4*2<dim_id<=NDIM==128
        ...
        if (query_id < n_query_desc) {
            ...
            res_distance [query_id * 2 + k] = sqrt(res_distance2_local[query_local_i * 2 + k]); // наконец извлекаем корень
        }
    }
}
