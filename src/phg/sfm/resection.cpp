...
namespace {
    ...
    cv::Matx34d estimateCameraMatrixDLT(const cv::Vec3d *Xs, const cv::Vec3d *xs, int count)
    {
        ...
        Eigen::VectorXd svd = svda.matrixV().col(svda.matrixV().cols() - 1);
        matrix34d result = matrix34d(
            svd[0], svd[1], svd[2], svd[3],
            svd[4], svd[5], svd[6], svd[7],
            svd[8], svd[9], svd[10], svd[11]);
    ...
    cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib, const std::vector<cv::Vec3d> &X, const std::vector<cv::Vec2d> &x)
    {
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            for (int i = 0; i < n_points; ++i) {
                cv::Vec3d p = calib.project(P * cv::Vec4d(X[i][0], X[i][1], X[i][2], 1));
                cv::Vec2d px = cv::Vec2d(p[0] / p[2], p[1] / p[2]); // спроецировать 3Д точку в пиксель с использованием P и calib;
