...
//            std::cout << "gauss: unique solution found" << std::endl;
    ...
    cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs, const std::vector<cv::Point2f> &points_rhs)
    {
        ...
        const float p = 0.95;
        const float w = 0.5;
        const int n_trials = log(1 - p) / log(1 - pow(w, n_samples));
...
{
    cv::Mat ans = T * cv::Point3d(pt.x, pt.y, 1.0);
    return cv::Point2d(ans.at<double>(0) / ans.at<double>(2), ans.at<double>(1) / ans.at<double>(2));
