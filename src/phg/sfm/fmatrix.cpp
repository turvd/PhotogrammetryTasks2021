...
//             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        D(0, 0) = svdf.singularValues().x();
        D(1, 1) = svdf.singularValues().y();
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        cv::Vec2d mean = sum / m_size;

        double sum_dist = 0;
        for (int i = 0; i < m_size; ++i) {
            cv::Vec2d dist = m[i] - mean;
            sum_dist += sqrt(dist.dot(dist));
        }
        double mean_dist = sum_dist / m_size / sqrt(2);
        return cv::Matx33d(
                1.0, 0.0, -mean(0),
                0.0, 1.0, -mean(1),
                0.0, 0.0, mean_dist
